    ...
    try:
        ...
        for product in products:
            ...
            for colorway in product.get('colorways', []):
                ...
                full_price = colorway.get('price', {}).get('fullPrice')
                in_stock = colorway.get('inStock')
                portrait_url = colorway.get('images', {}).get('portraitURL')

                unique_id = f"{pid}_{portrait_url}"

                cursor.execute("""
                    SELECT 1 FROM Informacjeoproduktach WHERE UnikalneID = ?
                """, (unique_id,))
                row = cursor.fetchone()

                if row:
                    cursor.execute("""
                        UPDATE Informacjeoproduktach
                        SET Nazwa = ?, Podtytuł = ?, Aktualna_cena = ?, Pelna_cena = ?, Dostepnosc = ?
                        WHERE UnikalneID = ?
                          AND (Nazwa != ? OR Podtytuł != ? OR Aktualna_cena != ? OR Pelna_cena != ? OR Dostepnosc != ?)
                    """, (title, subtitle, current_price, full_price, in_stock, unique_id,
                          title, subtitle, current_price, full_price, in_stock))
                else:
                    cursor.execute("""
                        INSERT INTO Informacjeoproduktach (Nazwa, Podtytuł, Aktualna_cena, Pelna_cena, UnikalneID, Dostepnosc)
                        VALUES (?, ?, ?, ?, ?, ?)
                    """, (title, subtitle, current_price, full_price, unique_id, in_stock))
        conn.commit()
        return True
